...
if(!user) {
  window.location.href = "http://127.0.0.1:5500/js-9/login.html"
}
...
favoriteIds = JSON.parse(favoriteIdsString);
...
function renderShows(filteredShows) {
  ...
  console.log("in render: ", filteredShows);
  console.log("fave: ", favoriteIds);
...
function toggleFavorite(id) {
  if (favoriteIds.includes(id)) {
    ...
    document.getElementById(`show-${id}`).classList.remove("fave");
  } else {
    favoriteIds.push(id);
    document.getElementById(`show-${id}`).classList.add("fave");
  }
  // renderShows(shows)
...
function filterShows() {
  ...
  // console.log(query);
  // const filteredShows = shows.filter((show) => show.name.includes(query));
  // console.log(shows);
...
// filtering using API request with query params
